...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
    ...
    it "knows Psyduck is the 54th pokemon" do
      # this passes but really it is asking for SELECT id FROM pokemon WHERE name = 'Psyduck'
      expect(@db.execute("SELECT * FROM pokemon WHERE id = '54'").flatten.first).to eq(54)
    end
